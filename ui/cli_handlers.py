...
import json
from messages.generic import handle_keypad_press, KEY_SET_INDEX_NAMES
...
def exec_phone_conference(ctx, clitext, argv, log):
    client = _require_client(ctx, log)
    if not client:
        return
    if len(argv) < 3:
        ...
        return
    number = argv[2]
    log(f"Conference with {number} ...")
    client.conference(number)
    log("")
...
def exec_show_calls(client, clitext, argv, log):
    ...
    log(f"{'Line':<6} {'CallId':<9} {'CallType':<13} {'CallState':<14} {'Time':<11} {'FromNum':<12} {'FromName':<15} {'ToNum':<12} {'ToName':<15}"    )
    log(f"{'----':<6} {'------':<9} {'--------':<13} {'---------':<14} {'----':<11} {'-------':<12} {'--------':<15} {'-----':<12} {'------':<15}"    )
    ...
    for call_id in call_ids:
        ...
        call_time_started = ci_mi_data.get("call_started", "")
        call_time_ended = ci_mi_data.get("call_ended", "")
        call_dur = client.state._human_elapsed(call_time_started, call_time_ended)
...
def exec_show_softkeys(client, clitext, argv, log):
    log("GLOBAL SOFTKEYS")

    log(f"{'Label':<10} {'Event':<6} {'Index':<6}")
    log(f"{'-----':<10} {'-----':<6} {'-----':<6}")

    for k, v in client.state.softkey_template.items():
        # log(f"{v}")
        label = v.get("label", "")
        keyevent = v.get("event", "")
        log(f"{label:<10} {keyevent:<6} {k:<6}")

    log("\nCURRENT SOFTKEY SET")
    sk_set = client.state.selected_softkey_set
    log(f"{KEY_SET_INDEX_NAMES.get(sk_set, 'UNKNOWN')} ({sk_set})")

    log("\nSOFTKEY SET DEFINITION")
    log(f"{'Label':<10} {'Event':<6} {'|':<3} {'Tmpl Name':<17} {'TIdx':<6} {'Info Name':<17} {'IIdx':<6}")
    log(f"{'-----':<10} {'-----':<6} {'+':<3} {'---------':<17} {'----':<6} {'---------':<17} {'----':<6}")
    sk_def = client.state.softkey_set_definition.get(str(sk_set), {})
    for k, v in sk_def.items():
        ...
        templ_data = client.state.softkey_template.get(str(template_index), {})
        label = templ_data.get("label", "")
        event = templ_data.get("event", "")

        log(f"{label:<10} {event:<6} {'|':<3} {template_index_name:<17} {template_index:<6} {template_info_name:<17} {info_index:<6}")
    log("")
